...
### Project-Level Configuration (IMPORTANT)

**All Rainbow workflow files must be created at the PROJECT level, NOT in the global `~/.claude/` directory.**

| Path Type | Example | Correct? |
|-----------|---------|----------|
| Project-level | `/path/to/project/.claude/teams/rainbow-workflow/` | ✅ Yes |
| Global | `~/.claude/teams/rainbow-workflow/` | ❌ No |

This ensures:
- Each project has its own isolated workflow state
- Multiple projects can run Rainbow workflows independently
- Workflow state is version-controllable with the project
...
Track workflow progress in `<project-root>/.claude/teams/rainbow-workflow/progress.json`:
...
**IMPORTANT**: All files must be created at the PROJECT level, under `<project-root>/.claude/teams/rainbow-workflow/`.

Before spawning any agents, create the tracking files:

#### 2a. Create team.json (Agent Team Configuration)

Create `<project-root>/.claude/teams/rainbow-workflow/team.json` with confirmed models:
...
Create `<project-root>/.claude/teams/rainbow-workflow/progress.json` with initial state:
...
1. **Project-level configuration**: All team files go in `<project-root>/.claude/`, NOT `~/.claude/`
2. **Confirm models first**: Ask user to confirm model for each agent before creating team
3. **Create team.json before spawning**: Record confirmed model assignments in team.json
4. **Verify agent startup**: Test each agent responds before proceeding with workflow
5. **One agent per workspace**: Each agent has its own git worktree
6. **Sync via remote**: Agents push/pull to share changes
7. **Track progress**: Main agent updates progress.json after each phase
8. **Discard incomplete changes on recovery**: Fresh worktree on restart
9. **Sequential phases**: Most phases depend on previous outputs
10. **Parallel when possible**: issue-manager, e2e-tester can run in parallel

## Red Flags

| Thought | Reality |
|---------|---------|
| "I'll put files in ~/.claude" | Files must be project-level. Use `<project-root>/.claude/`. |
